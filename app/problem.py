...
class Heat():
    ...
    '''
    left_x = None       # ліва границя області по х
    right_x = None      # права границя області по х

    start_t = None      # start of time
    end_t = None        # end of time count
    
    left_boundary = None      # left boundary condition
    right_boundary = None     # right boundary condition
    initial = None            # initial condition
    exact_solution = None           # solution for checking an algorithm

    alpha = None            # coefficient in the equation
    fn = None               # component of heterogeneousity
    _text = None             # things for formatted printing
    ...
    '''
        ...
        try:
            ...
            self.initial = smp.lambdify((x, t), json_data["initial condition"])
            if json_data['solution'] is not None:
                self.solution = smp.lambdify((x, t), json_data["solution"])
